...
Наследованию функциональных интерфейсов возможно, но конечный интерфейс все равно должен содержать только один нереализованный метод
...
Принимает Supplier<T>, который генерирует бесконечную последовательность на основе переданного ему функционального интерфейса.
...
* Метод iterate()
Работает с UnaryOperator, он схож с iterate, но принимает 2 аргумента: первый, с которого начинается генерация последовательности, а второй UnaryOperator который указывает принцип генерации новых значений.
```java
Stream.iterate(9, x -> x * x)
           .limit(4)
           .forEach(System.out::println);
```
Вывод в консоль будет выглядеть:
```java
9
81
6561
...
```
## 52. Функциональная композиция
Функциональная композиция позволяет объединять несколько функций в одну
```java
Predicate<String> startA =text->text.startsWith("A");
Predicate<String> endsB =text->text.endsWith("B");

Predicate<String> test =text->startA.test(text) && endsB.test(text);
String temp="AB";
System.out.println(test.test(temp));//true
```
Мы создаем 2 функциональных интерфейса, один проверяет что строка начинается с "A", а другой что заканчивается на "B", после мы их объеденили в test, в итоге если у нас строка подходит под 2 условия мы получим true

Методы композиции Predicate:

Метод and()
```java
Predicate<String> test =startA.and(endsB);
```
Результат будет такой же

Метод or()
```java
Predicate<String> test =startA.or(endsB);
```
Получим true если хоть один метод вернет true
...
Function<Integer,Integer> test =add.compose(multiply);
System.out.println(test.apply(2));//7
```
Вначале вызывается то что было передано как параметр,а после возвращенное значение передается на метод, который вызвал compose()

Метод andThen()
```java
Function<Integer,Integer> test =add.andThen(multiply);
```
Работает по обратной логике

Преимущества композиции:
* Читаемость
* Модульность: каждый блок это простая реализация, которую можно использовать для составных функций или отдельно повторно
* Тестируемость: так как каждая функция маленькая и стабильная
* Поддерживаемость: можно легко изменить одну функцию или композицию чем большой монолитный код
## 53. StreamAPI
поток — это последовательность элементов, призван решить проблему написания многословного и императивного кода для распространённых задач обработки данных

В Java8 появилась возможность создавать потоки из примитивов, так как мы не можем передать в качестве параметра примитивов с дженериками, появились 3 новых интерфейса IntStream, LongStream, DoubleStream.

Метод range(int startInclusive, int endExclusive): создает упорядоченный поток от первого до второго параметра с шагом 1, в результат не включается последний параметр, он является верхней границей.

Метод  rangeClosed(int startInclusive, int endInclusive): делает тоже самое, но включает второй элемент.
```java
IntStream intStream = IntStream.range(1, 3);
LongStream longStream = LongStream.rangeClosed(1, 3);
```
Random позволяет генерировать потоки примитивов:
```java
Random random = new Random();
DoubleStream doubleStream = random.doubles(3);
```
Мы также можем создать поток из String при помощи метода chars(), поскольку CharStream нет, мы используем IntStream
```java
IntStream streamOfChars = "abc".chars();
```
#### Поток файлов
Мы можем создать поток Stream<String> текстового файла с помощью lines():
```java
...
```
* Промежуточные операции являются ленивыми. Они фактически не обрабатывают данные до тех пор, пока не будет вызвана терминальная операция. Они преобразуют поток в другой поток.
* Терминальные операции являются жадными. Они запускают фактическую обработку потока и возвращают результат, не являющийся потоком (например, счётчик, список, необязательное значение или побочный эффект). После вызова терминальной операции поток считается использованным и не может быть использован повторно.

Для выполнения последовательности операций над элементами источника данных и агрегации их результатов нам необходимы три части: источник , промежуточная(ые) операция(и) и конечная операция.

Для каждого потока можно использовать только одну терминальную операцию.
...
промежуточные операции, уменьшающие размер потока, должны располагаться перед операциями, применяемыми к каждому элементу. Поэтому нам нужно разместить такие методы, как skip(), filter() и distinct(), в начале нашего потокового конвейера.

##### Метод reduce()

Есть 3 вида этого метода, они могут иметь следующие параметры:
* Идентификатор – начальное значение для аккумулятора или значение по умолчанию, если поток пуст и нечего накапливать
* Аккумулятор – функция, определяющая логику агрегации элементов. Поскольку аккумулятор создаёт новое значение на каждом шаге редукции, количество новых значений равно размеру потока, и только последнее значение является полезным.
* Функция Combiner – агрегирует результаты аккумулятора. Мы вызываем Combiner только в параллельном режиме для объединения результатов аккумуляторов из разных потоков.

##### Метод collect()

Сокращение потока также может быть выполнено другой терминальной операцией — методом collect(). Он принимает аргумент типа Collector
...
API позволяет создавать параллельные потоки, выполняющие операции в параллельном режиме. Если источником потока является коллекция или массив , это можно сделать с помощью метода parallelStream() :
```java
...
Одно из средств синхронизации это Семафоры которые располагаются в пакете java.util.concurrent, работает при помощи счсетчиков если счетчик равен нулю, поток блокируется, когда счетчик больше нуля поток получает доступ к ресурсу и счетчик уменьшается на единицу, после окончания работы счетчик увеличивается на единицу и семафор освобождается

```java
Semaphore(int permits)
Semaphore(int permits, boolean fair)
```
Параметр permits указывает на количество допустимых разрешений для доступа к ресурсу, параметр fair указывает будет ли соблюдаться порядок получения доступа, если false порядок неопределенный, если true в порядке, в каком они запрашивали доступ.
...
После работы нужно освободить разрешение пир помощи метода release()
```java
void release()
...
## 20. Atomic
## 21. Виртуальные поток
Виртуальный поток это легковесный поток, который управляется виртуальной машиной, одна виртуальная машина может поддерживать миллионы таких потоков, они подходят для операций которые продолжительное время блокируются(ввод-вывод, обращение к файлам, отправка сетевых запросов), они не предназначены для длительных операций которые нагружают процессор.

Как они работают:

Виртуальный поток не может работать сам по себе поэтому он прикрепляется к платформенному потоку, который еще называют носителем, когда виртуальный поток блокируется он открепляется от носителя, его состояние сохраняется в куче и носитель может обрабатывать другие виртуальные потоки,как только поток завершает блокирующую операцию он снова прикрепляет виртуальный поток на доступный платформенный для продолжения выполнения, таким образом платформенные потоки постоянно выполняют полезную работу.

Основные преимущества:
* Масштабируемость
* Простота кода: виртуальные потоки позволяют писать привычный код используя api
* Совместимость: виртуальные потоки это реализация Thread что позволяет писать совместимый код с существующим кодом и библиотеками.
...
Виртуальные потоки не имеют имени, если использовать метод getName() мы получим пустую строку

Виртуальные потоки являются потомками-демонами

Виртуальный потоки имеют фиксированный приоритет(5) 

Когда лучше их использовать:
* приложения с большим количеством блокирующих операций такие как веб-серверы, микросервисы, приложения для работы с бд
* Высоконагруженные системы которым необходимо обрабатывать много одновременных подключений

Когда не стоит использовать виртуальные потоки:
* для задач которые используют процессор например для вычисления математических расчетов
* Вызовы нативных методов могут блокировать несущий поток.
## 21. Lock
Альтернатива  synchronized это блокировки

Поток пытается получить доступ к общему ресурсу, если ресурс свободен, он блокируется, после завершения работы блокировка с ресурса снимается, если поток обращается к ресурсу и он уже заблокирован он переходит в состояние ожидания, пока не будет снята блокировка.

Классы блокировок реализуют интерфейс Lock,который реализует методы:
