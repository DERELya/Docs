...
Процесс добавления нового элемента:
  ...
  Если несколько элементов могут оказаться в одной группе (из-за коллизий hashCode()), тогда и только тогда вызывается equals() для сравнения нового элемента с существующими элементами в этой конкретной группе. Если equals() возвращает true для любого существующего элемента, новый элемент считается дубликатом и не добавляется.
...
Представляет собой двустороннюю очередьв которой можно добавлять и удалять элементы с двух сторон.
Реализация:
* LinkedList
* ArrayDeque
```java
...
## 45. Stack
Стек это коллекция в которой элементы добавляются и удаляются в вершине. Принцип называется(FILO) первым пришел, последний ушел( примером из жизни будет стопка тарелок, первую мы возьмем сверху, которую последнюю положили)
...
```
## 46. Iterator
Интерфейс Iterator это объект который позволяет перебирать коллекцию по одному элементу за раз. Чтобы его использовать вы должны получить экземпляр итератора из коллекции объектов, он отслеживает элементы базовой коллекции, если изменить коллекцию во время перебора итератор это заметить и выбросит исключение при следующей попытке получить следующий элемент из итератора.
...
Синтаксис: `(аргументы)->{тело}`. Лямбда-выражения могут обращаться к переменным,но главное условие чтобы они были объявлены как final.Если локальная переменная никогда не переназначается после инициализации, компилятор считает её «фактически финальной», и вы можете использовать её в лямбда-выражении без явного объявления её как final. 
...
* Метод с predicate
Для примера возьмем метод класса stream filterкоторый принимает predicate, а возвращает stream только с теми элементами, которые подходят под условие predicate.
```java
...
Подобно  директиве exports , мы объявляем пакет экспортируемым. Но мы также перечисляем модули, которым разрешено импортировать этот пакет, в директиве require . Давайте посмотрим, как это выглядит:
```java
module my.module {
...
Обратите внимание, что имя класса, которое мы  используем, — это либо интерфейс, либо абстрактный класс службы, а не класс реализации :
```java
module my.module {
    uses class.name;
}
...
## 12.	Многопоточность
## 13. Способы создания потоков
## 14. Виды состояния потоков
## 15. Ключевое слово volatile
##16. Проблемы многопоточных приложений: состояние гонки,  дедлоки и лайфлоки
## 17. Пакет java.util.concurrent
## 18. Atomic
## 19. Lock
## 20. Executors
## 21. Future, CompletableFuture
## 22. Синхронизаторы
